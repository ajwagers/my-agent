...
class MemoryStore:
    ...
    Uses DefaultEmbeddingFunction for consistent vector space.
    """
...
- `DefaultEmbeddingFunction` (all-MiniLM-L6-v2) is used everywhere — `rag_data` and `agent_memory` share the same vector space, so recall results are directly comparable to RAG results.
...
def _chroma_modules():
    ...
    mock_ef.DefaultEmbeddingFunction.return_value = mock_ef_instance
    return {
...
from chromadb.utils.embedding_functions import DefaultEmbeddingFunction
c = chromadb.HttpClient(host='chroma-rag', port=8000)
col = c.get_or_create_collection('agent_memory', embedding_function=DefaultEmbeddingFunction())
